...
4. `main.rs` builds a single shared `reqwest::Client` and calls `scheduler::spawn_all` (passing `config.debug` through), which spawns one tokio task per `CheckConfig`.
5. Each task loops: wait for its `interval` tick, run the check for its `mode`, translate the result into a `kuma::PushStatus`, and call `kuma::push`.
...
- `main() -> anyhow::Result<()>` (async, `#[tokio::main]`) -- initializes logging, parses CLI args, loads config, logs every check when `config.debug` is set, builds the shared `reqwest::Client` (with a desktop-Chrome `User-Agent` override -- see below), spawns the scheduler with `config.debug`, waits on `tokio::signal::ctrl_c()`, then aborts all check task handles before returning.
...
- `Config { debug: bool, checks: Vec<CheckConfig> }` -- `debug` uses `#[serde(default)]` (defaults to `false` when absent).
...
- `spawn_all(checks: Vec<CheckConfig>, client: Client, debug: bool) -> Vec<JoinHandle<()>>` -- spawns one `tokio::spawn`ed task per check (each with its own cloned `Client`) and returns their handles. `debug` is threaded through to every push.

Key internal functions:

- `run_check_loop(check: CheckConfig, client: Client, debug: bool)` -- owns a `tokio::time::interval(check.interval)` (with `MissedTickBehavior::Delay`, so a slow check run doesn't cause a burst of catch-up ticks) and calls `run_once` on every tick, logging (not propagating) any error so one bad run doesn't kill the loop.
