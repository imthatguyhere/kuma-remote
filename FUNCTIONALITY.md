...
6. `main.rs` builds a single shared `reqwest::Client` with a connect timeout and an overall request timeout (so a stalled GitHub/Kuma response can never hang startup or a push indefinitely). If `config.auto_update` is set, it then calls `updater::check_and_update` with that client, `config.service_mode`, and the single-instance lock before doing anything else -- see `updater.rs` below. On an applied update this exits the process (self-spawning a replacement only when the single-instance lock is actually held; otherwise, including under `service_mode`, it just exits and relies on a supervisor to restart it); otherwise it logs and falls through to normal startup regardless of outcome.
...
- `HANDSHAKE_TIMEOUT: Duration = 200ms` -- connect/read timeout used when a challenger probes the occupant of the lock port for `LOCK_MAGIC`.
- `SPAWN_ATTEMPTS: u32 = 3`, `SPAWN_RETRY_DELAY: Duration = 250ms` -- retry budget for spawning the replacement process in `try_update`, covering a transient failure to launch the just-replaced executable (e.g. a brief antivirus scan lock).
...
- `try_update(client: &Client, service_mode: bool, instance_lock: &mut Option<TcpListener>) -> anyhow::Result<()>` (async) -- the full check/update flow:
  ...
  4. Hashes the running exe's own bytes (`sha2::Sha256` over `tokio::fs::read` of `current_exe()`) and compares (case-insensitive) to the remote digest; logs and returns `Ok(())` if they match.
  ...
  6. Writes the verified bytes to a sibling temp path (`exe_path.with_extension("exe.new")`, same directory as the running exe so the replace stays on one filesystem, via `tokio::fs::write`), calls `self_replace::self_replace` on it, then best-effort deletes the temp file (`tokio::fs::remove_file`).
  7. If `service_mode` is true, or `instance_lock` is `None` (lock disabled or unavailable), logs (with an extra `warn!` in the latter case explaining why self-spawn is being skipped) and calls `std::process::exit(0)` immediately -- no replacement spawned, no lock touched.
  8. Otherwise (lock held), spawns a new process at the same exe path with the same CLI args (`std::env::args_os().skip(1)`), retrying up to `SPAWN_ATTEMPTS` times (`SPAWN_RETRY_DELAY` apart) if the spawn itself fails. If every attempt fails, logs an `error!` (distinct from the routine `warn!` skip cases, since this means an already-verified update is stuck) and returns the `Err` (propagated up to `check_and_update`; this process keeps running on its current in-memory code, still holding its single-instance lock -- the file on disk is already updated for whenever it's next restarted by any means). Only once a spawn attempt has succeeded does it call `instance_lock.take()` to release the single-instance port -- the replacement's own claim attempt at its startup tolerates the brief remaining delay before this process fully exits (see `CLAIM_ATTEMPTS`/`CLAIM_RETRY_DELAY`) -- then calls `std::process::exit(0)`; this function does not return past that point.
