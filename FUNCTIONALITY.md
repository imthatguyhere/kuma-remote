...
> Module list adapted from the CLAUDE.md template: this project has no `installed.rs`/`cdk_info.rs` (those belong to a different, software-inventory project). Sections below cover this project's actual modules: `main.rs`, `config.rs`, `scheduler.rs`, `checks/ping.rs`, `checks/heartbeat.rs`, `checks/web.rs`, `kuma.rs`, `logging.rs`, `updater.rs`.
...
3. `config::Config::load` reads the file, deserializes it as StrictYAML into `Config`, normalizes it, and validates it (non-empty, unique `id`s, non-zero `interval`s, `host` present for `ping` checks, `url` present for `web` checks).
...
7. `main.rs` builds two shared `reqwest::Client`s, both with the same `config.http_user_agent`, `config.http_connect_timeout`, and `config.http_timeout` (so a stalled GitHub/Kuma/checked-site response can never hang startup, a push, or a check run indefinitely; the release-asset download in `updater.rs` bounds itself separately, see below): a strict `client` used for everything, and a `lenient_client` that additionally sets `.danger_accept_invalid_certs(true)`, used only as a fallback by `web` checks (see `checks/web.rs`). If `config.auto_update` is set, it then calls `updater::check_and_update` with the strict client, `config.service_mode`, the `lock_unavailable` flag, the single-instance lock, and `config.slow_download_mode` before doing anything else — see `updater.rs` below. If the result is `UpdateOutcome::Exit` (an update was applied; self-spawning a replacement only when the single-instance lock is actually held, otherwise — including under `service_mode` — relying on a supervisor to restart it), `main.rs` returns `Ok(())` immediately, so the process exits via its own normal return path; on `UpdateOutcome::Continue` it falls through to normal startup regardless of what happened during the check.
8. `main.rs` calls `scheduler::spawn_all` (passing both clients, `config.debug`, and `config.report_run_failures` through), which spawns one tokio task per `CheckConfig`.
9. Each task loops: wait for its `interval` tick, run the check for its `mode` (`ping` requires `host` and reports `Up`/`Down` based on the ping result; `heartbeat` always reports `Up` with message `"Heartbeat"`, optionally including a latency if `host` is set and the ping succeeds; `web` requires `url`, sends it a GET request, and reports `Up`/`Down` based on the response — see `checks/web.rs`), translate the result into a `kuma::PushStatus`, and call `kuma::push`. If the run itself errors out (rather than completing with an `Up`/`Down` result), the error is logged and, when `report_run_failures` is set, also pushed to Kuma as a `down` status with the error as `msg`.
...
Single StrictYAML file. Path is set via `--config`, or else resolved by `main::resolve_config_path` to the first existing file among `kuma-remote.yaml`, `kuma-config.yaml`, `config.yaml` (falling back to `kuma-remote.yaml` if none exist, so the load error names a sensible file). Schema: a top-level `checks` sequence of mappings, each deserialized into `config::CheckConfig`. `interval` is parsed via `humantime_serde` from strings like `"30s"`/`"5m"`/`"1h"`. `url` (required for `CheckMode::Web`) and `test_string` (optional, only meaningful for `CheckMode::Web`) are otherwise unused by `Ping`/`Heartbeat`. Deserialization uses `strict_yaml_rust::serde::from_str`. `auto_update` (`bool`, default `true` via `default_auto_update`) gates whether `updater::check_and_update` runs at all; `service_mode` (`bool`, default `false`) and `instance_lock`/`instance_lock_port` (`bool`/`u16`, defaults `true`/`51247` via `default_instance_lock`/`default_instance_lock_port`) control the single-instance guard and the updater's restart strategy; `slow_download_mode` (`bool`, default `false` via `#[serde(default)]`) controls how the update download is time-bounded — see `updater.rs`. `http_user_agent` (`String`, default a desktop-Chrome-on-Windows string via `default_http_user_agent`), `http_connect_timeout`, and `http_timeout` (both `Duration` via `humantime_serde`, defaults `7s`/`30s` via `default_http_connect_timeout`/`default_http_timeout`) configure the two shared `reqwest::Client`s built in `main.rs`. `Config::validate` rejects `instance_lock_port == 0` while the lock is actually in effect (`instance_lock` true and `service_mode` false) as a hard startup error (not just a warning): port `0` always binds to a fresh OS-assigned port on every attempt, so it can never detect a duplicate instance, silently defeating the lock rather than just weakening it. It also rejects a zero `http_connect_timeout` or `http_timeout`, either of which would time out every connection/request instantly.
...
Key detail: both `reqwest::Client`s are built with `.user_agent(config.http_user_agent.clone())`, defaulting to a real desktop Chrome-on-Windows string instead of reqwest's default `reqwest/x.y.z`. Some reverse proxies / WAFs (e.g. Cloudflare bot protection) block generic HTTP-client user agents while allowing browsers, which otherwise manifests as push requests failing (404/403) even though the same URL works from a browser. Both also set `.connect_timeout(config.http_connect_timeout)` and `.timeout(config.http_timeout)` (defaults 7s/30s), so a stalled response from GitHub's API, Kuma itself, or a `web`-checked site can never hang a caller (notably `updater::check_and_update`, which runs before any check task is spawned) indefinitely; the release-asset download in `updater.rs` doesn't use `client`'s timeout at all, instead bounding itself per-chunk in `download_with_progress` (see `DOWNLOAD_HARD_TIMEOUT`/`DOWNLOAD_STALL_TIMEOUT`), relying on periodic progress logging in the meantime rather than a single short deadline. `lenient_client` additionally sets `.danger_accept_invalid_certs(true)` and is otherwise unused outside `checks/web.rs`'s certificate-failure fallback — never used for the push URL or the updater, both of which should keep failing hard on a bad certificate.
...
- `Config { debug: bool, report_run_failures: bool, auto_update: bool, service_mode: bool, instance_lock: bool, instance_lock_port: u16, slow_download_mode: bool, http_user_agent: String, http_connect_timeout: Duration, http_timeout: Duration, checks: Vec<CheckConfig> }` — `debug`, `service_mode`, and `slow_download_mode` use `#[serde(default)]` (default `false`); `report_run_failures` uses `#[serde(default = "default_report_run_failures")]` (defaults to `true`); `auto_update` uses `#[serde(default = "default_auto_update")]` (defaults to `true`); `instance_lock` uses `#[serde(default = "default_instance_lock")]` (defaults to `true`); `instance_lock_port` uses `#[serde(default = "default_instance_lock_port")]` (defaults to `51247`); `http_user_agent` uses `#[serde(default = "default_http_user_agent")]` (defaults to a desktop-Chrome-on-Windows string); `http_connect_timeout`/`http_timeout` use `#[serde(with = "humantime_serde", default = "default_http_connect_timeout"/"default_http_timeout")]` (default `7s`/`30s`).
- `CheckConfig { id: String, name: String, mode: CheckMode, host: Option<String>, push_url: String, url: Option<String>, test_string: Option<String>, interval: Duration }` — `interval` uses `#[serde(with = "humantime_serde")]`; `host` uses `#[serde(default)]` since it's only required for `CheckMode::Ping`; `url` uses `#[serde(default)]` since it's only required for `CheckMode::Web`; `test_string` uses `#[serde(default)]` and is only meaningful for `CheckMode::Web`.
- `CheckMode` — enum with variants `Ping`, `Heartbeat`, and `Web` (`#[serde(rename_all = "lowercase")]`, so the YAML scalars `ping`/`heartbeat`/`web` map to them). New check modes extend this enum.
...
- `default_http_user_agent() -> String` — serde default-value function for `Config::http_user_agent`; returns the desktop-Chrome-on-Windows string previously hardcoded in `main.rs`.
- `default_http_connect_timeout() -> Duration` — serde default-value function for `Config::http_connect_timeout`; returns `7s`.
- `default_http_timeout() -> Duration` — serde default-value function for `Config::http_timeout`; returns `30s`.
...
- `spawn_all(checks: Vec<CheckConfig>, client: Client, lenient_client: Client, debug: bool, report_run_failures: bool) -> Vec<JoinHandle<()>>` — spawns one `tokio::spawn`ed task per check (each with its own cloned `client`/`lenient_client` pair) and returns their handles. `lenient_client` is only used by `CheckMode::Web` runs (see `checks/web.rs`); `debug` and `report_run_failures` are threaded through to every check loop.

Key internal functions:

- `run_check_loop(check: CheckConfig, client: Client, lenient_client: Client, debug: bool, report_run_failures: bool)` — owns a `tokio::time::interval(check.interval)` (with `MissedTickBehavior::Delay`, so a slow check run doesn't cause a burst of catch-up ticks) and calls `run_once` on every tick, logging (not propagating) any error so one bad run doesn't kill the loop. When `report_run_failures` is set, a run error is also pushed to Kuma as `PushStatus::Down { message: "Check run failed: {err}" }`; a failure to push that is itself logged (and swallowed).
- `run_once(check: &CheckConfig, client: &Client, lenient_client: &Client, debug: bool) -> anyhow::Result<()>` — matches on `check.mode`: `CheckMode::Ping` unwraps `check.host` (present per `Config::validate`), runs `ping::ping_once`, and maps its `Up`/`Down` outcome straight to `PushStatus::Up { ping_ms: Some(_), message: None }` / `PushStatus::Down`; `CheckMode::Heartbeat` calls `heartbeat::beat_once(check.host.clone())` and always builds `PushStatus::Up { ping_ms: <from the outcome>, message: Some("Heartbeat") }`; `CheckMode::Web` unwraps `check.url` (present per `Config::validate`), runs `web::check_once(client, lenient_client, &check.id, &url, check.test_string.as_deref())`, and maps its outcome to `PushStatus::Up { ping_ms: Some(_), message: Some(<web message>) }` / `PushStatus::Down`. Either way, the outcome is logged and the resulting `kuma::PushStatus` is pushed.
...
Purpose: the `web` check mode — a single GET request per run, classified `Up`/`Down` from the response.

Types:

- `WebOutcome` — `Up { latency_ms: f64, message: String }` (`message` is the status code, plus which `test_string` matched when one was configured, e.g. `200 ("Welcome" matched)`, or just the status code, e.g. `200`, otherwise) or `Down { reason: String }`.

Public functions:

- `check_once(strict_client: &Client, lenient_client: &Client, check_id: &str, url: &str, test_string: Option<&str>) -> anyhow::Result<WebOutcome>` (async) — sends a GET to `url` via `strict_client`. If that request fails and `url` starts with `https://`, retries once via `lenient_client` (which accepts invalid certificates); a `warn!` (with `check_id`, `url`, and the strict-client error) is logged if the retry succeeds, since that means the server's certificate is invalid, expired, or self-signed. If both fail (or the strict request fails on a non-`https` `url`), returns `Down { reason: "Request failed: {err}" }` immediately. On any successful response: if `test_string` is `Some`, the body is always read and checked — regardless of status, since even a non-2xx response has a body worth checking — and it's `Up { message: "{status} (\"{needle}\" matched)", .. }` only if the status is *also* 2xx; any other combination (bad status, no match, or both) is `Down { reason: "{status} (\"{needle}\" matched|not matched)" }`, naming both signals in the reason either way. If `test_string` is `None`, returns `Up { message: "{status}", .. }` iff the status is 2xx, else `Down { reason: "HTTP {status}" }`. `latency_ms` is measured from just before the initial request to just after the response headers are in hand (before any body read).

Key algorithm: a certificate failure on an `https` `url` never fails the check on its own — the check is still evaluated against whatever response the lenient retry got, since the goal is reachability, not certificate validity. The bad-certificate warning is logged independently of the up/down outcome, so it's visible even when the check still reports `Up`.
